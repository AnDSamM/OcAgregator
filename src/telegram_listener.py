...
from config import API_ID, API_HASH, PHONE_NUMBER, YC_FOLDER_ID, YC_API_KEY, BOT_TOKEN
...
async def normal_handler(event):
    try:
        ...
        if message.sender_id == normal_handler.my_id:
            print(f"   Пропущено (собственное сообщение)")
            return
        ...
        if "Найдено сообщение по вашей подписке" in message.text:
            print(f"   Пропущено (уведомление бота)")
            return
